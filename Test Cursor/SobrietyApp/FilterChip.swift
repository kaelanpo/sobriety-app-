        ...
        Button(action: action) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 20)
                    .fill(isSelected ? DS.ColorToken.purpleLight.opacity(0.15) : DS.ColorToken.card)
            )
            .overlay(
                RoundedRectangle(cornerRadius: 20)
                    .stroke(isSelected ? AnyShapeStyle(DS.ColorToken.purpleGradient) : AnyShapeStyle(Color.clear), lineWidth: 2)
            )
            .foregroundStyle(isSelected ? AnyShapeStyle(DS.ColorToken.purpleGradient) : AnyShapeStyle(DS.ColorToken.textPrimary))
        }
